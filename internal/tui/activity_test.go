...
func capture(content string) backend.ScreenCapture {
	return backend.ScreenCapture{Content: content, OK: true}
}
...
func TestActivityTrackerUpdate(t *testing.T) {
	...
	t.Run("screen changed above threshold marks working", func(t *testing.T) {
		...
		tr.Update(map[string]backend.ScreenCapture{
	...
	t.Run("preserves tool on capture failure", func(t *testing.T) {
		...
		tr.Update(map[string]backend.ScreenCapture{}, nil, []string{"c1"}, now)
